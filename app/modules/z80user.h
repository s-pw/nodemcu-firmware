 ...
 * the end of the file for an example for running CA80.
 *
...
typedef struct CA80 {
    ...
    unsigned char keypad[4]; //rows
    unsigned char PB;
    unsigned char PC;
    unsigned char* memory_pages[0x10000 / PAGE_SIZE];
} CA80;

extern unsigned char ca80_in(CA80 *m, unsigned short port);
extern void ca80_out(CA80 *m, unsigned short port, unsigned char val);
extern unsigned char ca80_read_byte(CA80 *ca80, unsigned short addr);
extern void ca80_write_byte(CA80 *ca80, unsigned short addr, unsigned char val);

#define Z80_READ_BYTE(address, x)                                       \
{                                                                       \
    (x) = ca80_read_byte((CA80 *) context, address);                    \
}


#define Z80_READ_WORD(address, x)                                       \
{                                                                       \
    (x) = ca80_read_byte((CA80 *) context, address) | (ca80_read_byte((CA80 *) context, (address)+1) << 8); \
}

#define Z80_WRITE_BYTE(address, x)                                      \
{                                                                       \
    ca80_write_byte((CA80 *) context, address, x);                      \
}

#define Z80_WRITE_WORD(address, x)                                      \
{                                                                       \
	ca80_write_byte((CA80 *) context, address, x);                      \
	ca80_write_byte((CA80 *) context, (address)+1, (x) >> 8);           \
}
...
{                                                                       \
        (x) = ca80_in((CA80 *) context, port);				            \
}

#define Z80_OUTPUT_BYTE(port, x)                                        \
{                                                                       \
        ca80_out((CA80 *) context, port, x); 			                \
}
